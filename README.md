...
200 WebP frames (100 on phones) from `public/frames/`, which is drawn on a canvas. The footage is portrait, so
phones fill the screen, while desktops show a sharp viewfinder panel over a blurred copy of the same frame. The
camera overlay (timecode, beat, frame counter, brew temperature, progress bar) and the four copy blocks all read
